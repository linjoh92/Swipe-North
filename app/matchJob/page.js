...
export default function matchJobList() {
  ...
  return (
    <div style={{ backgroundColor:'#e6e6e6', height: '100%', top: '100px' , minHeight: 'calc(100vh - 124px)' , position: 'absolute', left: '0', right: '0'}}>
      <div className={styles.matchListContainer}>
        <div className={styles.matchadeListNavBar}>
          <p className={styles.matchedNavBar}>{text.match}</p>
          <Link href='/appliedJobs'><p className={styles.searchedNavBar}>{text.searched}</p></Link>
        </div>
        <div>
          {jobInfo.map((match) => (
